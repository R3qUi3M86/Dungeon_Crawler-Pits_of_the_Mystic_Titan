        ...
        if movement_collision_sprite_group == unique_player_objects.PLAYER_SHADOW_SPRITE_GROUP:
            ...
            if unique_player_objects.PLAYER_SHADOW_SPRITE.rect.colliderect(movement_collision_sprite_group.sprite.rect):
                ...
                if mask_collision_coordinates != None:
                    ...
                    adjust_player_movement_vector(mask_collision_coordinates, True)

def player_vs_level_movement_collision():
    global collision_correction_vector
    for movement_collision_sprite_group in entity_manager.level_collision_sprite_groups:
        if unique_player_objects.PLAYER_SHADOW_SPRITE.rect.colliderect(movement_collision_sprite_group.sprite.rect):
            mask_collision_coordinates = pygame.sprite.collide_mask(unique_player_objects.PLAYER_SHADOW_SPRITE, movement_collision_sprite_group.sprite)
            if mask_collision_coordinates != None:
                collision_correction_vector = get_correction_vector(mask_collision_coordinates, movement_collision_sprite_group.sprite, True)
...
def adjust_player_movement_vector(mask_collision_coordinates, level_collision = False):
    global speed_vector
    global acceleration_vector

    if level_collision == False:
...
def get_correction_vector(mask_collision_coordinates, movement_collision_sprite = None, level_collision=False):
    ...
    if level_collision == False:
        ...
        if speed_vector[0] >= 0 and speed_vector[1] == 0:
            #Player moving east or north east or south east
            pass
        #while pygame.sprite.collide_mask(unique_player_objects.PLAYER_SHADOW_SPRITE, movement_collision_sprite) != None:
